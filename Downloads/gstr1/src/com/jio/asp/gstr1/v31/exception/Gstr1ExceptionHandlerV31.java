 ...
 */
@ControllerAdvice
public class Gstr1ExceptionHandlerV31 {
	private static final Logger log = LoggerFactory.getLogger(Gstr1ExceptionHandlerV31.class);
	@Autowired
	...
	public ResponseEntity<Map<String, Object>> exceptionHandler(HttpServletRequest request, Exception ex) {
		...
		exObject.put(Gstr1ConstantsV31.ERROR_CODE, ErrorCodesV31.GENERIC_STATUS_CODE);
		exObject.put(Gstr1ConstantsV31.ERROR_DESC, convertExceptionClass(ex));
		exObject.put(Gstr1ConstantsV31.ERROR_GRP, Gstr1ConstantsV31.GENERIC);
	...
	private String convertExceptionClass(Exception ex) {
		...
		if (ex != null && ex.getClass() != null) {
			name = messageSourceV31.getMessage("IDENTIFIED_ERROR", new Object[] { ex.getClass().getSimpleName() },
					LocaleContextHolder.getLocale());
		}
		if (StringUtils.isEmpty(name) || StringUtils.equals("Unknown", name)) {
			name = messageSourceV31.getMessage("UNIDENTIFIED_ERROR", null, LocaleContextHolder.getLocale());
		}
		return name;
	}
